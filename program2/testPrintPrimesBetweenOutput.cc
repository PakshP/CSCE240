...
// initial tests to compare output generated by PrintPrimesBetween
// to expected output
#include<iostream>
using std::cout;
using std::endl;
...
int main() {
  system("./testPrintPrimesBetweenCall > student-primes-output.txt");
  ifstream student_file("student-primes-output.txt");
  if ( !student_file.good() )
    cout << "Issue with student output. Check student-primes-output.txt"
         << endl;
  ifstream correct_file("expected-primes-output.txt");
  if ( !correct_file.good() )
    cout << "Issue with expected output. Confirm that the file "
         << "\"expected-primes-output.txt\" is saved in the current directory "
